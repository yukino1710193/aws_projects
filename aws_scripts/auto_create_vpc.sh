...
# Private
    aws ec2 create-route --route-table-id $ROUTE_TABLE_PRIVATE_NAME --destination-cidr-block 0.0.0.0/0 --nat-gateway-id $NATGW_ID --region $REGION
    
    # Associate subnet with custom route table to make public
# Public
    aws ec2 associate-route-table  --region $REGION --subnet-id $SUBNET_PUB_ID --route-table-id $ROUTE_TABLE_PUBLIC_ID
# Private
    aws ec2 associate-route-table  --region $REGION --subnet-id $SUBNET_PRI_ID --route-table-id $ROUTE_TABLE_PRIVATE_ID
    ## Configure subnet to issue a public IP to EC2 instances
# Public
    aws ec2 modify-subnet-attribute --region $REGION --subnet-id $SUBNET_PUB_ID --map-public-ip-on-launch
# Private
    aws ec2 modify-subnet-attribute --region $REGION --subnet-id $SUBNET_PRI_ID --no-map-public-ip-on-launch
...
aws ec2 delete-subnet --subnet-id $SUBNET_PRI_ID --region $REGION
aws ec2 delete-subnet --subnet-id $SUBNET_PUB_ID --region $REGION
aws ec2 delete-route-table --route-table-id $ROUTE_TABLE_PRIVATE_ID --region $REGION
aws ec2 delete-route-table --route-table-id $ROUTE_TABLE_PUBLIC_ID --region $REGION
aws ec2 detach-internet-gateway --internet-gateway-id $IGW_ID --vpc-id $VPC_ID --region $REGION
aws ec2 delete-internet-gateway --internet-gateway-id $IGW_ID --region $REGION
aws ec2 delete-nat-gateway --nat-gateway-id $NATGW_ID --region $REGION
aws ec2 release-address --allocation-id $EIP_NAT_ID --region $REGION
