...
# Querry id
    ...
    SUBNET_PUB_ID=$(aws ec2 describe-subnets --filters "Name=tag:Name,Values=SUBNET_PUB" --query "Subnets[0].SubnetId" --output text --vpc-id $VPC_ID --region $REGION_ID)
    SUBNET_PRI_ID=$(aws ec2 describe-subnets --filters "Name=tag:Name,Values=SUBNET_PRI" --query "Subnets[0].SubnetId" --output text --vpc-id $VPC_ID --region $REGION_ID)
    ...
    ROUTE_TABLE_PUBLIC_ID=$(aws ec2 describe-route-tables --vpc-id $VPC_ID --filters "Name=tag:Name,Values=$ROUTE_TABLE_PUBLIC_NAME" --query "RouteTables[0].RouteTableId" --output text --region $REGION_ID)
    ROUTE_TABLE_PRIVATE_ID=$(aws ec2 describe-route-tables --vpc-id $VPC_ID --filters "Name=tag:Name,Values=$ROUTE_TABLE_PRIVATE_NAME" --query "RouteTables[0].RouteTableId" --output text --region $REGION_ID)
